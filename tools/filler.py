...
def fill_user_description(ctx: RunContext[MyDeps], preferences: None | dict = None, performance: None | dict = None) -> None | str:
    ...
    Args:
        ...
        - performance (dict) : a dictionary containing the performance of the user, with keys like kilometer_per_day and positive_height_difference_per_day, and values being integers
    Returns:
        Nothing
    Examples:
        ```python
        fill_user_description(preferences = {amenity: ["restaurant", "pub"], natural: ["coastline", "beach"]}, performance = {kilometer_per_day: 100})
        fill_user_description(performance = {positive_height_difference_per_day: 100, kilometer_per_day: 40})
        ```
    """
