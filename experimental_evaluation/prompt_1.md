...
Goal: Generate multiple routes option and help the user to choose the preferred one.
...
3. Use `fill_trip_description` to select the route, among the candidates, that the user prefer.
...
### Transition
- The user decide that does not want to add any amenities to the trip.
...
3. If the error still cannot be resolved, suggest the user to try again later and terminate.
